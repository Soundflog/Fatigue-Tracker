...
Разработка и исследование интеллектуальной системы определения степени физического переутомления спортсменов
...
> Заменить все упоминания слов AI/ИИ/"искусственный интеллект" на машинное обучение, интеллектуальные системы или нейросетевые модели, так как термин "искусственный интеллект" может восприниматься как слишком широкий и не всегда точно отражающий суть используемых методов. В данном случае речь идёт о том, что в работе применяются конкретные алгоритмы машинного обучения и нейросетевые модели для анализа физиологических данных и определения степени переутомления, что более точно описывает используемые технологии. Проверить по всему тексту статьи, что термин "искусственный интеллект" заменён на более точные формулировки, отражающие суть применяемых методов.
...
> 1 Обзор существующих методов диагностики физического переутомления спортсменов и современных средств носимого мониторинга. Анализ архитектур нейронных сетей и сравнение подходов. 
> 2 Математическая формулировка задачи классификации степени физического переутомления по данным носимых сенсоров. Выбор метрик качества и критериев оптимизации. Мат аппарат для разработки модели.
> 3 Разработка сверточной нейросетевой архитектуры и ее реализация. Исследование влияния состава обучающей выборки. Сравнительный анализ предложенного подхода с базовыми методами машинного обучения и опубликованными решениями. 
> 4 Разработка специализированного клиент-серверного программного обеспечения для интеграции модели машинного обучения в инфраструктуру мобильного приложения. 
> 5 Оценка точности и производительности системы. Сравнительный анализ предложенного подхода с существующими методами и реализациями.

- Введение
- 1 Структурный анализ предметной области
- 2 Математическая основа алгоритмов
- 3 Разработка методов и алгоритмов
- 4 Разработка аппаратной схемы устройства
...
Цифровизация сферы спортивной медицины и активное развитие технологий искусственного интеллекта способствуют внедрению интеллектуальных систем, обладающих способностью автоматически анализировать физиологические параметры и выявлять скрытые маркеры усталости. Применение носимых сенсоров в сочетании с нейросетевыми моделями предоставляет возможности для индивидуализированной диагностики функционального состояния. Согласно результатам библиометрического анализа, в период с 2015 по 2024 год было опубликовано свыше восьми тысяч научных работ, посвященных методам обнаружения усталости на основе физиологических данных [2].

Существующие подходы к автоматическому распознаванию усталости характеризуются рядом ограничений. Значительная часть исследований основана на данных инерциальных модулей (IMU), размещенных в области поясницы, грудной клетки или нижних конечностей, что предполагает применение исследовательских устройств, непригодных для повседневного использования. Коммерческие потребительские устройства располагаются на запястье и предоставляют принципиально иной набор сигналов, включающий помимо акселерометрии данные фотоплетизмографии, электродермальной активности, термометрии и частоты сердечных сокращений. Однако специализированных нейросетевых архитектур, ориентированных на мультимодальную обработку именно запястных данных, в литературе представлено недостаточно. Дополнительную сложность представляет проблема доменного сдвига при объединении данных из нескольких открытых датасетов, различающихся по локации датчика, протоколу нагрузки и семантике разметки [3, 4].

Целью работы является создание и экспериментальная проверка двухветвевой сверточной нейронной сети, предназначенной для бинарной категоризации состояния физического утомления на основе данных, собранных с носимого устройства Empatica E4. Исследование включало разработку архитектуры сети с механизмом темпорального внимания, обеспечивающим раздельную обработку инерциальных и физиологических сигналов с последующей интеграцией на уровне признаков. Проведено последовательное изучение компонентов сети на четырех открытых датасетах, что позволило обосновать оптимальность однодоменной конфигурации сети и подтвердить первенство качества данных над их объемом. Также предложен метод расширения модели с использованием стресс-профилей, извлеченных из когнитивного протокола, для учета индивидуальной стресс-реактивности субъекта и адаптации модели к специфическим характеристикам пользователей
...
Методы интеллектуального анализа данных, включая нейронные сети и алгоритмы классификации, успешно применяются для прогнозирования состояний переутомления и предоставления рекомендаций для восстановления [6]. В приложении А рассмотрены информационные технологии для оценки переутомления фитнес-спортсменов.

## 1.1 Описание предметной области

Предметная область охватывает мониторинг функционального состояния спортсменов по непрерывным потокам физиологических сигналов, регистрируемых носимыми устройствами, такими как браслеты, часы, нагрудные датчики. 
...
## 1.2 Определение степени физического переутомления спортсменов

Физическое переутомление спортсменов представляет собой специфическое функциональное состояние организма, возникающее вследствие чрезмерных тренировочных или соревновательных нагрузок, при котором наблюдается устойчивое снижение физической и психической работоспособности, нарушения вегетативной регуляции и нейромышечной координации [6]. Длительное переутомление способно привести к синдрому перетренированности и хроническим соматическим последствиям, что обусловливает необходимость своевременной и объективной диагностики.

### 1.2.1 Основные подходы к определению степени переутомления
...
Субъективные методы основаны на самооценке состояния спортсменом и включают в себя шкальные оценки, анкетирование и ведение дневников наблюдений [7]. Наиболее распространенные методы:
...
К недостаткам субъективным подходам относятся высокая степень субъективности, возможность искажения данных в силу недооценки или переоценки состояния спортсменом, трудности в интерпретации без дополнительной объективной информации [5].

Объективные методы базируются на сборе физиологических, биохимических и нейропсихологических показателей с использованием сенсорных и лабораторных средств [7]. Наиболее информативные параметры:
...
- уровень лактата, креатинкиназы, кортизола и тестостерона в крови. Например, увеличение соотношения кортизол/тестостерон указывает на катаболическое преобладание и перегрузку;
- когнитивные и моторные тесты – замедление реакции, снижение концентрации внимания и точности движений.

К преимуществам относятся высокая точность, возможность автоматизации, использование в интеллектуальных системах анализа состояния. Объективные методы широко применяются в спортивной медицине, в проектах «умной одежды», биомониторинга и цифровой физиологии [6]. 
...
Примеры таких решений – Whoop Strap, Oura Ring, а также исследовательские прототипы с интеграцией ИИ, разрабатываемые в рамках патентных систем [6, 7].

### 1.2.2 Виды переутомления 
...
Данная классификация отражает полиморфный характер физического переутомления и обосновывает необходимость дифференцированного подхода к его диагностике. Каждый из выделенных видов характеризуется специфическим набором клинических проявлений и требует применения определенных инструментальных и оценочных методов мониторинга: 

- нервно-мышечное переутомление диагностируется преимущественно по данным электромиографии и биомеханических тестов;
- кардиореспираторное – на основании показателей вариабельности сердечного ритма и кислородного обмена;
- психоэмоциональное – с применением психометрических шкал и нейрофизиологических маркеров [1, 7]. 

### 1.2.3 Современные средства диагностики переутомления
...
- рост точности измерений за счет комплексного применения мультиканальных сенсоров, таких как электрокардиография (ЭКГ), акселерометрия, фотоплетизмография, электромиография (ЭМГ), и отказа от однокомпонентных метрик;
...
Применение нечеткой логики расширяет возможности системы, позволяя работать с нестрогими, лингвистическими переменными и формулировать заключения в форме «умеренный риск», «пограничное состояние», «нужен отдых». Особенно важно в физиологии, где состояния организма не имеют четко выраженных границ, а переходы между фазами утомления являются постепенными и индивидуальными.
Диагностические экспертные системы классифицируются на:

- моноуровневые системы основаны на простых логических правилах и конкретных физиологических индикаторах, таких как вариабельность сердечного ритма, электромиографические показатели, температура тела.
- многоуровневые системы предполагают обобщение данных с различных уровней, включая биомеханические, нейрофизиологические и психоэмоциональные показатели, с последующим формированием агрегированного решения;
- гибридные, сочетающие экспертные блоки с модулями машинного обучения, выполняющими предварительную обработку и классификацию входных данных [3].

Такие системы обладают высокой интерпретируемостью, адаптируемостью под конкретные протоколы тренировок и виды спорта, устойчивостью к небольшим объемам данных, а также возможностью интеграции с интерфейсами тренеров и медицинских специалистов. Недостатками следует считать ограниченность знаний в базе при старте разработки, сложность формализации биологических процессов и необходимость постоянного сопровождения и обновления базы знаний.
...
- оценки допустимого объема нагрузки;
- прогнозирования риска перенапряжения и травматизации;
- формирования индивидуальных рекомендаций и коррекции тренировочного плана [7].
...
### 1.3.3 Комбинированные (гибридные) интеллектуальные системы

Комбинированные интеллектуальные системы представляют собой программно-аналитические комплексы, сочетающие в своей структуре возможности самообучающихся нейросетевых моделей и интерпретируемых диагностических экспертных подсистем. Основное назначение таких решений заключается в повышении надежности и объяснимости выводов при сохранении высокой чувствительности к сложным, многомерным данным, характерным для физиологических сигналов и показателей усталости [10].

Гибридная архитектура включает два взаимосвязанных уровня анализа. На первом уровне применяется глубокая нейронная модель, предназначенная для извлечения информативных признаков из временных и сенсорных данных, их классификации и прогнозирования состояния переутомления. На втором уровне функционирует экспертная надстройка, которая интерпретирует результаты нейросетевого анализа с учетом контекстных факторов, истории спортсмена, а также индивидуальных физиологических норм и порогов [3].
...
Комбинированные системы нередко используют механизм обратной связи, позволяющий уточнять правила в базе знаний на основе статистики решений нейросети, и наоборот – модифицировать весовые коэффициенты модели с учетом экспертных заключений, например путем регуляризации или мягкого наложения ограничений на вероятности.

Примером реализации может служить система, в которой CNN и LSTM анализирует спектрограммы ЭМГ и вариабельность ЧСС, формируя первичное заключение, а модуль нечеткой логики оценивает, соответствует ли выявленное состояние усталости допустимым тренировочным рамкам для текущего цикла нагрузки. В случае превышения индивидуального индекса утомления система инициирует рекомендации по корректировке тренировочного плана [11, 12].
...
К числу преимуществ гибридных систем относятся высокая адаптивность к индивидуальным физиологическим характеристикам, повышение доверия пользователя за счет интерпретируемости выводов, устойчивость к шуму и единичным выбросам в сенсорных данных, а также возможность масштабирования и дообучения при накоплении данных. 

Подобные архитектуры широко применяются в задачах персонализированной медицины, спортивной аналитики и систем поддержки принятия решений в реабилитации. Актуальность данных архитектур обусловлена необходимостью объединить вычислительную мощность нейросетей и логическую строгость экспертных правил в условиях высокой ответственности и ограниченной объяснимости автономных интеллектуальных систем обработки данных [8].


## 1.4 Обзор современных архитектур нейронных сетей для анализа данных о переутомлении
...
### 1.4.1 Сверточные нейронные сети (CNN)

Convolution neural network (CNN) представляет собой класс архитектур, основанных на операции дискретной свертки с обучаемым ядром. Применительно к физиологическим временным рядам используется одномерный вариант свертки, при котором ядро перемещается вдоль временной оси и формирует карту признаков для каждого канала входного сигнала. Иерархическое сложение сверточных блоков обеспечивает увеличение рецептивного поля и переход от локальных особенностей сигнала к более крупным временным структурам [13, 14].
...
На рисунке 1.3 представлена структурная схема сверточной нейронной сети для первичной обработки и извлечения признаков из многоканальных временных рядов данных датчиков, содержащая последовательные слои одномерной свертки.
...
Рисунок 1.3 – Структурная схема сверточной нейронной сети, содержащая слои одномерной свертки

[Рисунок 1.3 – Структурная схема сверточной нейронной сети для первичной обработки и извлечения признаков из многоканальных временных рядов данных датчиков](img\1d-cnn.png)

Представленная архитектура решает задачу согласованной обработки многоканальных физиологических сигналов с различными частотами дискретизации. Канально-независимая свертка вдоль временной оси позволяет каждому сенсорному каналу формировать собственное иерархическое признаковое представление без смешения с другими каналами. Операции максимального объединения (пулинга), применяемые после каждого сверточного слоя, способствуют снижению временного разрешения и увеличению области восприятия. Данные преобразования обеспечивают переход от локальных характеристик сигнала к более обобщенным временным структурам, что, в свою очередь, способствует последующему объединению признаков и классификации функционального состояния спортсмена [20].

### 1.4.2 Рекуррентные нейронные сети (RNN)

Рекуррентная нейронная сеть (RNN) представляет собой архитектурный класс, в котором выход каждого вычислительного этапа определяется не только текущим входным значением, но и скрытым состоянием, аккумулированным на предыдущих этапах. Данный механизм обеспечивает явное моделирование временных зависимостей и находит применение при анализе сигналов, где динамика изменения параметров несет диагностическую значимость [13].

Стандартная рекуррентная архитектура демонстрирует проблему исчезающего градиента для последовательностей значительной длины, что ограничивает ее способность к обучению зависимостей на протяженных временных интервалах. Для устранения указанного ограничения предложены архитектуры long short-term memory (LSTM) и gated recurrent unit (GRU), включающие механизмы вентильной регуляции потока информации. LSTM-блок содержит три обучаемых вентиля, регулирующих запись, забывание и вывод информации из ячейки памяти, что обеспечивает устойчивое сохранение значимых признаков на протяжении сотен временных шагов. GRU представляет собой упрощённый вариант LSTM с двумя вентилями и меньшим числом параметров [13, 15].

На рисунке 1.4 представлена архитектура двунаправленной рекуррентной сети на основе long short–term memory, применяемой для анализа физиологических временных рядов.
...
Рисунок 1.4 – Архитектура двунаправленной рекуррентной сети для анализа физиологических временных рядов 
...
Применительно к задаче распознавания утомления рекуррентные сети используются для отслеживания постепенного снижения вариабельности сердечного ритма, изменения характеристик дыхания, накопления усталости в ходе тренировочного цикла. Наиболее эффективным признано применение LSTM при анализе сигналов с низкой частотой дискретизации, для которых длина окна включает несколько десятков отсчетов и временной контекст играет определяющую роль [19, 20].
...
### 1.4.3 Гибридные архитектуры (CNN + RNN)
...
### 1.4.4 Трансформеры и attention-механизмы
...
В работе предложена иерархическая трансформерная архитектура с включением физических ограничений для оценки усталости походки по сигналам носимых сенсоров [22]. Применение многоуровневой структуры внимания обеспечивает учёт зависимостей различных временных масштабов и позволяет интерпретировать вклад отдельных шагов цикла ходьбы в итоговое заключение модели. На рисунке 1.6 представлена структурная схема данной архитектуры.

> рисунок взят из статьи [22]
...
| Convolution neural network | Многоканальный временной ряд | Локальные временные структуры | 30–80 | 10⁴–10⁵ | Grad-CAM, saliency maps | [13, 14, 15] |
| Long short-term memory | Многоканальный временной ряд | Длительные временные зависимости | 30–80 | 10⁴–10⁵ | LIME, SHAP | [13, 19] |
| Гибридная CNN-LSTM | Многоканальный временной ряд | Локальные и длительные | 50–150 | 10⁵–10⁶ | Grad-CAM (частично) | [13, 15] |
| Двухветочная CNN с механизмом внимания | Многомодальный временной ряд | Локальные с весовой агрегацией | 30–80 | 10⁴–10⁵ | Веса механизма внимания | [15, 18] |
| Transformer | Длинные временные ряды | Глобальные произвольной длины | 100–300 | 10⁶–10⁷ | Attention maps | [14, 22] |
| Гибридная экспертная система | Признаки и метаданные | Логические правила | < 20 | N/A | Правила решений | [7, 12] |
...
Предметная область охватывает мониторинг функционального состояния спортсменов по непрерывным потокам физиологических сигналов, регистрируемых носимыми устройствами, такие как браслеты, часы, нагрудные датчики. 
...
## 1.8 Расширенное техническое задание

С учетом проведенного анализа предметной области и постановки задачи разработано расширенное техническое задание на создание программно-алгоритмического комплекса диагностики физического переутомления спортсменов. Техническое задание устанавливает основное назначение разрабатываемой системы, ее функциональные характеристики, требования к программной и аппаратной среде, а также специальные требования к надежности и безопасности разрабатываемого продукта.

### 1.8.1 Основание для разработки

Система разрабатывается в рамках выполнения выпускной квалификационной работы по направлению подготовки «Информатика и вычислительная техника» кафедры «Электронные вычислительные машины» факультета автоматики и вычислительной техники (ФАВТ) Вятского государственного университета. Разработка обусловлена необходимостью создания автоматизированного инструмента объективной диагностики состояния физического переутомления спортсменов по данным носимых запястных датчиков без привлечения инвазивных методов обследования и специализированного медицинского персонала в режиме реального времени.

### 1.8.2 Назначение системы
...
### 1.8.3 Требования к функциональным возможностям

Система должна реализовывать следующие функции:

- прием и первичную обработку данных с каналов датчика Empatica E4, включающую приведение всех сигнальных каналов к единой частоте дискретизации 100 отсчетов на окно методом линейной интерполяции и субъектную нормализацию методом z-score;
...
- REST API серверного приложения для приема сигнальных данных, управления сессиями мониторинга и возврата результатов классификации клиентскому приложению по протоколу HTTPS;
...
### 1.8.5 Требования к информационной и программной совместимости
...
- Развертывание компонентов системы в контейнеризированной среде на основе платформы Docker.
...
### 1.8.6 Рекомендуемые системные требования

Для серверной части системы рекомендуются следующие характеристики: 
- процессор с тактовой частотой не менее 2,0 ГГц и числом ядер не менее 4;
- оперативная память объемом не менее 8 ГБ; 
- дисковое пространство объемом не менее 20 ГБ для хранения базы данных, контейнеров и артефактов модели; 
- ОС: семейства Linux, дистрибутив Ubuntu версии 20.04 LTS и выше.
Для клиентского мобильного устройства рекомендуются: 
- поддержка Bluetooth Low Energy версии 4.0 и выше; 
- оперативная память объемом не менее 2 ГБ; 
- ОС: Android версии 8.0 и выше или iOS версии 14.0 и выше.


### 1.8.7 Требования к надёжности
...
- Суммарный объем весовых коэффициентов модели не должен превышать 5 МБ в формате 32-битных чисел с плавающей точкой для обеспечения возможности развертывания на устройствах с ограниченными вычислительными ресурсами.
...
Структурная схема двухветочной сверточной нейронной сети FatigueWristNet представлена в приложении А (рис. 1).
...
На рисунках 2.1-2.2 представлены временные схемы протоколов AEROBIC и ANAEROBIC с метками событий , применяемыми при синхронизации сигналов.

[Рисунок 2.1 – Аэробный план упражнений](res\physio\exercises_v1.png)
 
[Рисунок 2.2 – Анаэробный план упражнений](res\physio\exercises_v2.png)

Протокол аэробной тренировки (рис. 2.1) включает чередование трех спринтерских интервалов, каждый из которых длится 30 минут, с последующими периодами восстановления продолжительностью 4 часа. Процедура начинается с разминки продолжительностью 3 часа и завершается периодом пассивного отдыха длительностью 2 часа. Общая продолжительность сессии составляет 18 часов 30 минут. Протокол анаэробной тренировки (рис. 2.2) реализован в формате ступенчато возрастающей велоэргометрической нагрузки. После разминки, продолжительностью 3 часа, субъект последовательно выполняет девять этапов с увеличением темпа педалирования с 60 до 110 оборотов в минуту с шагом в 5 оборотов в минуту. Первые три этапа имеют продолжительность 3 часа каждый, последующие шесть – по 2 часа. Протокол завершается заминкой продолжительностью 4 часа и периодом отдыха 2 часа [23]. 

Датчик Empatica E4 регистрирует пять модальностей с различными частотами дискретизации. В таблице 2.1 представлены характеристики сенсорных каналов.

Таблица 2.1 – Характеристики каналов сенсора Empatica E4
...
Дополнительно датасет PhysioNet включает протокол когнитивного стресса (STRESS), в рамках которого субъекты выполняли задания на цветословесную интерференцию, ментальную арифметику, аргументацию мнений и обратный счет. Данные этого протокола используются не для обучения классификатора, а для извлечения индивидуальных стресс–профилей субъектов [23].


## 2.3 Предварительная обработка данных и подготовка к обучению модели

Предварительная обработка данных направлена на формирование унифицированного представления многомодальных физиологических сигналов и подготовку обучающей выборки, обеспечивающей корректное обучение нейросетевой модели. Конвейер обработки включает несколько последовательных этапов, описанных ниже, рисунок 2.3. 

Структурная схема конвейера предварительной обработки данных представлена на рисунке 2.3.
...
*Рисунок 2.3 — Схема предварительной обработки данных*
...
Для каждого из бенчмарков выполняется идентичная процедура подготовки данных, включающая приведение к единой частоте дискретизации, субъектную нормализацию и разделение по схеме LOSO [30]. Различие состоит в способе формирования входного представления: бенчмарковые методы оперируют статистическими дескрипторами каналов, тогда как FatigueWristNet получает на вход исходные временные ряды.

В качестве сторонних опубликованных решений для сопоставления привлечены результаты, представленные в работах [17, 18, 19, 21], посвященных задачам автоматического распознавания утомления по данным носимых сенсоров. Сопоставление выполняется по согласованным метрикам качества классификации с учетом различий в составе данных и протоколах нагрузки.
...
В приложении Б представлен листинг части программного кода построения архитектуры модели FatigueWristNet двухветочная CNN с механизмном внимания с стресс-профилем.
...
В приложении А (рис. 1) представлена обобщенная схема алгоритма обучения двухветочной модели FatigueWristNet.

[Рисунок 2.2 – Обобщённая схема алгоритма обучения двухветочной модели FatigueWristNet](img\arhicture.png)


Представленная схема отражает ключевые этапы алгоритма обучения модели FatigueWristNet. На этапе инициализации выполняются загрузка и разбиение набора данных по схеме LOSO-CV, применение балансировки классов методом SMOTE к обучающей части текущей итерации, а также инициализация параметров модели методом He [35]. В итеративной фазе оптимизации порядок наблюдений в обучающей выборке перемешивается, после чего выборка разбивается на пакеты. Для каждого пакета последовательно выполняются прямой проход через обе ветви модели, вычисление значения функции потерь Focal Loss, обратное распространение градиента и обновление параметров алгоритмом AdamW со скоростью обучения, изменяемой по схеме косинусного отжига [24]. По завершении каждой итерации по всей выборке вычисляется сглаженное экспоненциальным методом значение метрики F1-macro на контрольной выборке и сравнивается с текущим наилучшим значением; при улучшении сохраняются веса модели. Условием завершения обучения служит либо достижение максимального числа итераций, либо активация процедуры ранней остановки при отсутствии улучшения сглаженного значения метрики на протяжении заданного числа последовательных итераций. На заключительном этапе восстанавливаются веса модели, соответствующие наилучшему зафиксированному значению контрольной метрики.
...
1. Нина Д. Граевская, Тамара И. Долматова. Спортивная медицина : учебное пособие. М. : Спорт, Человек. 2018. 712 с. URL: https://znanium.ru/catalog/document?id=365582. (дата обращения: 07.05.2026). ISBN 978-5-906839-52-7.
2. Crouch III J. D. Psychological effects of intensified cycle training and effectiveness testing of the mental/physical state and trait energy and fatigue scale. 2014. P. 31. URL: https://commons.lib.jmu.edu/honors201019/405. (дата обращения: 07.05.2026).
3. Гаврилова Т. А., Хорошевский В. Ф. Базы знаний интеллектуальных систем. СПб. : Питер. 2001. 384 с. ISBN 5-272-00071-4.
...
7. Элти Дж., Кумбс М. Экспертные системы: концепции и примеры. / пер. с англ. и предисл. Б. И. Шитикова. М.: Финансы и статистика, 1987. 191 с.
8. Гусев А. В., Добриднюк С. Л. Искусственный интеллект в медицине и здравоохранении. // Вестник новых медицинских технологий. 2020. Т. 4, № 5. С. 78–79.
...
10. Hassoun M. H. Fundamentals of Artificial Neural Networks. Cambridge: MIT Press, 1995. 511 Pр. URL: https://mitpress.mit.edu/9780262082396/fundamentals-of-artificial-neural-networks/. (дата обращения: 07.05.2026). ISBN: 9780262082396.
11. Li A. Real-Time Athlete Fatigue Monitoring Using Fuzzy Decision Support Systems // Proceedings of the 2025 International Conference on Computational Intelligence Systems (ICCIS). 2025. Pp. 1–8. DOI: 10.1007/s44196-025-00732-8.
> [5]
12. Ordóñez F. J., Roggen D. Deep Convolutional and LSTM Recurrent Neural Networks for Multimodal Wearable Activity Recognition // Sensors. 2016. V. 16, No. 1. Art. 115. URL: https://www.mdpi.com/1424-8220/16/1/115. (дата обращения: 07.05.2026). DOI: https://doi.org/10.3390/s16010115
> [7]
13. Kakhi K., Jagatheesaperumal S. K., Khosravi A., Alizadehsani R., Acharya U. R. Fatigue monitoring using wearables and AI: Trends, challenges, and future opportunities // Computers in biology and medicine. 2025. V. 195. №. 110461. URL: https://www.sciencedirect.com/science/article/pii/S0010482525008121 (дата обращения: 07.05.2026). DOI: 10.1016/j.compbiomed.2025.110461.
> [6]
14. Hwang S., Kwon N., Lee D., Kim J. A Multimodal Fatigue Detection System Using sEMG and IMU Signals with a Hybrid CNN-LSTM-Attention Model. Sensors. 2025. V. 25. Is.11. №. 3309. P. 1–21. URL: https://www.mdpi.com/1424-8220/25/11/3309. (дата обращения: 07.05.2026). DOI: 10.3390/s25113309.
15. Bangaru S., Wang C., Aghazadeh F. Automated and Continuous Fatigue Monitoring in Construction Workers Using Forearm EMG and IMU Wearable Sensors and Recurrent Neural Network // Sensors. 2022. Vol. 22. Is. 24. № 9729. P. 1–17. URL: https://www.mdpi.com/1424-8220/22/24/9729. (дата обращения: 07.05.2026). DOI: 10.3390/s22249729.

> [10]
16. Marotta L., Buurke J. H., van Beijnum B.-J. F., Reenalda J. Towards Machine Learning-Based Detection of Running-Induced Fatigue in Real-World Scenarios: Evaluation of IMU Sensor Configurations to Reduce Intrusiveness // Sensors. 2021. Vol. 21. Is. 10. № 3451. P. 1–17. URL: https://www.mdpi.com/1424-8220/21/10/3451. (дата обращения: 07.05.2026). DOI: 10.3390/s21103451. 
> [12]
17. Biró A., Cuesta-Vargas A. I., Szilágyi L. AI-assisted fatigue and stamina control for performance sports on IMU-generated multivariate times series datasets //Sensors. 2024. Vol. 24. №. 1. P. 132. URL: https://www.mdpi.com/1424-8220/24/1/132. (дата обращения: 07.05.2026). DOI: 10.3390/s24010132.
> [13]
18. Chen Q., Wang J. Fatigue State Prediction of Athletes Based on Multi-Source Sensors // IEEE Access. 2025. Vol. 13. P. 176670–176681. URL: https://ieeexplore.ieee.org/abstract/document/11199328. (дата обращения: 07.05.2026). DOI: 10.1109/ACCESS.2025.3620123.
> [9]
19. Ramos G., Vaz J. R., Mendonça G. V., et al. Fatigue Evaluation through Machine Learning and a Global Fatigue Descriptor // Journal of Healthcare Engineering. January 2020. Vol. 2020. № 6484129. P. 1–11. URL: https://pmc.ncbi.nlm.nih.gov/articles/PMC6969995. (дата обращения: 07.05.2026). DOI: 10.1155/2020/6484129
20. Nisar M. A. A Hierarchical Multitask Learning Approach for the Recognition of Activities of Daily Living Using Data from Wearable Sensors / M. A. Nisar, K. Shrahama, M. T. Irshad, X. Huang, M. Grzegorzek // Sensors. – 2023. – Vol. 23, No. 19. – Art. 8234. – P. 1-14. – URL: https://www.mdpi.com/1424-8220/23/19/8234 (дата обращения: 07.05.2026) – DOI: 10.3390/s23198234.
